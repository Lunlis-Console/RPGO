...
public partial class ItemsTabView : UserControl
{
    ...
    {
        InitializeComponent();
        TypeFilter.ItemsSource = new[] { "все" }.Concat(Ui.ItemTypes).ToList();
    ...
    {
        _dt = _db.LoadTable(@"SELECT id, name, type, value, damage_min, damage_max, defense, max_health_bonus, heal_amount, restore_mana, stock, description,
        ...
        Ui.Bind(Grid, _dt);
        Ui.ShowOnly(Grid, "id", "name", "type", "quest_item");
        Ui.MakeComboColumn(Grid, "type", Ui.ItemTypes);
        ApplyFilter();
    ...
    private void ApplyFilter()
    {
        ...
        string filter = "";
        if (type != "все") filter = $"type = '{type.Replace("'", "''")}'";
