...
public class TradeRequestWindow : GameWindow
{
    ...
    public override void Draw(SpriteBatch sb)
    {
        ...
        var aSize = font.MeasureString("Принять");
        sb.DrawString(font, "Принять", new Vector2(acceptRect.X + (btnW - aSize.X) / 2, acceptRect.Y + (btnH - aSize.Y) / 2), Color.White);
        ...
        var dSize = font.MeasureString("Отказаться");
        sb.DrawString(font, "Отказаться", new Vector2(declineRect.X + (btnW - dSize.X) / 2, declineRect.Y + (btnH - dSize.Y) / 2), Color.White);
    }
}
