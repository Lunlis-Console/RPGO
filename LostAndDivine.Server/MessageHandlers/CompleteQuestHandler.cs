...
public class CompleteQuestHandler : BaseHandler
{
    ...
    public override async Task Handle(ClientConnection connection, GameMessage message, Player? player)
    {
        ...
        {
            await SendError(connection, ErrorCodes.NotAtBoard, "¬ернитесь к доске заданий, чтобы сдать задание.");
            return;
        }

        if (questId == null)
        {
            await SendError(connection, ErrorCodes.QuestNotSpecified, "«адание не указано.");
            return;
        }
        ...
        {
            Type = "chat",
            Data = new { Name = "—истема", Text = result.Message }
        });
        ...
        if (result.LeveledUp)
        {
            ...
            await SendToClient(connection, GameMessage.SystemChat($"”ровень повышен! ¬ы теперь уровень {player.Level}! +{BalanceStatic.AttributePointsPerLevel} очков атрибутов.{skillMsg} HP восстановлены."));
        }
