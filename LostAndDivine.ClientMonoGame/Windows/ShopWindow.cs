    ...
    public override void Update(GameTime gameTime, KeyboardState keyboard, MouseState mouse)
    {
            ...
            for (int c = 0; c < GridCols; c++)
            {
                ...
                if (idx < FilteredItems.Count && rect.Contains(mouse.X, mouse.Y))
                {
                    ...
                    if (pressed && !filterConsumed)
                    {
                        ...
                        DragStateChanged?.Invoke(FilteredItems[idx]);
                    }
                }
            }
        }
        ...
        if (released && _dragIndex >= 0)
        {
            ...
            if (idx < FilteredItems.Count)
                DropOnInventory?.Invoke(new Point(mouse.X, mouse.Y), FilteredItems[idx]);
        }

        // ПКМ — покупка сразу; ЛКМ — только двойной клик (одиночный ЛКМ запускает drag)
        if (_hoverItem >= 0 && _hoverItem < FilteredItems.Count)
        {
            if (mouse.RightButton == ButtonState.Pressed && prevRightState == ButtonState.Released)
                RequestBuy(FilteredItems[_hoverItem], keyboard.IsKeyDown(Keys.LeftControl) || keyboard.IsKeyDown(Keys.RightControl));
            else if (pressed)
                HandleBuyClick(FilteredItems[_hoverItem]);
        }
    ...
    public override void Draw(SpriteBatch sb)
    {
        ...
        for (int r = 0; r < GridRows; r++)
        {
            for (int c = 0; c < GridCols; c++)
            {
                ...
                int idx = (r + _scrollOffset) * GridCols + c;
                bool filled = idx < FilteredItems.Count;
