...
public class StatusWindow : GameWindow
{
    ...
    public override void Draw(SpriteBatch sb)
    {
        ...
        int rowY = cy + 4;
        DrawText(sb, _data.Name ?? "????", infoX, rowY, TitleGold);
        rowY += 18;
        if (!string.IsNullOrEmpty(_data.ClassName))
        {
            DrawText(sb, _data.ClassName, infoX, rowY, new Color(160, 200, 220));
            rowY += 18;
        }
        DrawText(sb, $"Уровень {_data.Level}", infoX, rowY, StatColor);
        rowY += 18;
        DrawText(sb, $"Золото: {_data.Gold}", infoX, rowY, new Color(220, 200, 80));
        rowY += 18;
        DrawText(sb, $"Опыт: {_data.Experience}", infoX, rowY, DimColor);
        ...
        for (int i = 0; i < combat.Length; i++)
        {
            ...
            DrawText(sb, combat[i].Item2, (int)(rx + cw / 2 - 4 - valW), ry + 3, StatColor);
        }
        ...
        foreach (var slot in LostAndDivine.Shared.Models.EquipmentSlots.All)
        {
    ...
    private void DrawBreakdownRow(SpriteBatch sb, ref int cy, int cx, int cw, string name, BreakdownPart? p)
    {
        if (p == null) return;
        sb.Draw(SpriteCache.Pixel, new Rectangle(cx, cy, cw, RowH), RowBg);
        DrawText(sb, name, cx + 10, cy + 3, DimColor);
        DrawText(sb, $"баз {p.Base}", cx + 100, cy + 3, DimColor);
        if (p.WeaponDamageMax > 0)
        {
            string wDmg = p.WeaponDamageMin == p.WeaponDamageMax
                ? $"орж {p.WeaponDamageMax}"
                : $"орж {p.WeaponDamageMin}-{p.WeaponDamageMax}";
            DrawText(sb, wDmg, cx + 170, cy + 3, DimColor);
        }
        string parts = $"атр {p.AttrBonus} + экп {p.EquipBonus}";
        if (p.SkillBonus > 0) parts += $" + нв {p.SkillBonus}";
        DrawText(sb, parts, cx + 230, cy + 3, DimColor);
        DrawText(sb, $"= {p.Total}", cx + cw - 52, cy + 3, StatColor);
        cy += RowH;
    }
