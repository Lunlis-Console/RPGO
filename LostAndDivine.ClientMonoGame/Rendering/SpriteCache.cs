...
public static class SpriteCache
{
    ...
    public static void Load(GraphicsDevice device, Microsoft.Xna.Framework.Content.ContentManager content)
    {
        ...
        var spriteNames = new[]
        {
            ...
            "icon_communication", "icon_equipment", "icon_inventory", "icon_journal", "icon_settings", "icon_skills", "icon_status", "icon_mail", "icon_worldmap",
            "skill",
        ...
        foreach (var si in skillIcons)
            LoadTexture($"icon_skill_{si.ToLower()}", $"LostAndDivine.ClientMonoGame.Content.Sprites.Skill_Icons.Icon_{si}.png");

        // Анимации навыков
        var skillAnims = new[] { "StrongHand", "BarrageOfBlows", "Cutting", "HolyTrinity", "Duel" };
        foreach (var sa in skillAnims)
            LoadTexture($"skill_anim_{sa.ToLower()}", $"LostAndDivine.ClientMonoGame.Content.Sprites.Skill_Animations.{sa}-Sheet.png");

        // Анимации ловушек (Retreat)
        LoadTexture("hazard_retreat_acid", "LostAndDivine.ClientMonoGame.Content.Sprites.Skill_Animations.Retreat-1-Sheet.png");
        LoadTexture("hazard_retreat_smoke", "LostAndDivine.ClientMonoGame.Content.Sprites.Skill_Animations.Retreat-2-Sheet.png");
        LoadTexture("hazard_retreat_snare", "LostAndDivine.ClientMonoGame.Content.Sprites.Skill_Animations.Retreat-3-Sheet.png");

        // Спрайт стрелы для снарядов
        LoadTexture("projectile_arrow", "LostAndDivine.ClientMonoGame.Content.Sprites.Skill_Animations.Arrow.png");
    ...
    private static Texture2D? LoadTexture(string name)
    {
        ...
        try
        {
            ...
            var resName = $"LostAndDivine.ClientMonoGame.Content.Sprites.{name}.png";
    ...
    public static Texture2D? GetTileset(string tilesetId, int tileSize, out int cols, out int rows)
    {
        ...
        try
        {
            var resName = $"LostAndDivine.ClientMonoGame.Content.Tilesets.{tilesetId}.png";
            var asm = typeof(SpriteCache).Assembly;
            using var stream = asm.GetManifestResourceStream(resName);
            if (stream == null)
            {
    ...
    public static Texture2D? GetIconWorldMap() => Get("icon_worldmap");
